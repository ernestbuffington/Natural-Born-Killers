...
#pragma warning(disable : 4136)     // X86 no longer have to ignore this in MSVC 2015 removed by Ghost 5/5/2016
...
// microsoft's fabs use to seem to be ungodly slow, not true anymore in MSVC 2015...
...
typedef struct
{
	...
	byte runscale;		   // so we can mess with the running speed
} pmove_state_t;
...
#define	RF2_FLAMETHROWER	2048	// client-side flamethrower effects (damage done in game code)
...
#define	RDF_PAUSED			4		// so renderer knows if it's paused
...
// duplicated these for use in teamplay, since they aren't used at all in multi play
...
// dmflags->value flags
// disabled those that are not used in Kingpin
...
{
	// MDX, ENABLE the following line when .mdx system goes online
...
//	int		modelindex2, modelindex3, modelindex4;	// weapons, CTF flags, etc
	...
	// flamethrower (only used on client-side)
