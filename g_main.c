...
void ShutdownGame(void)
{
	...
	// KOOGLEBOT_ADD
	sprintf(buf1, "%d", level.num_botloads);
	gi.cvar_set("bots", buf1);
	// KOOGLEBOT_END
...
{
	char	*basevars[] = { "basedir", "cddir", NULL };	// console variables that point to possible file locations
...
/* EndDMLevel The timelimit or fraglimit has been exceeded */
...
void ExitLevel(void)
{
	...
	// KOOGLEBOT_ADD added 4/30/2016 by Ghost - Disconnect Bots when we exit a level!
	edict_t *bot;

	gi.dprintf("void ExitLevel(void) is being called!");
	...
	for (i = 0; i < maxclients->value; i++)
	{
		...
		if (bot->inuse)
		{
			if (bot->is_bot)
			{
				...
				//safe_bprintf(PRINT_MEDIUM, "%s disconnected\n", bot->client->pers.netname);
			}
		}
	}
	// KOOGLEBOT_ADD added 4/30/2016 by Ghost - Disconnect Bots when we exit a level!
...
void G_RunFrame(void)
{
	...
	if ((int)timelimit->value && !(level.framenum % 10)) 
	{
		...
		if (!level.intermissiontime && ((level.modeset == MATCH) || (level.modeset == TEAMPLAY))) 
		{
	...
	sprintf(buf2, "%d", level.num_botloads);
	gi.cvar_set("bots", buf2);
