...
char lockpvs[8],scaletime[8],locktex[8],lockfoot[8],lockmouse[8],lockrecoil[8],lockangle[8],ver_check[8]; //FREDZ add recoil, angle
...
//	sl_Logging( &gi, NULL );	// Standard Logging
	...
	gi.dprintf("-------------------------------------\n");
	gi.dprintf("   Init Game Natural Born Killers    \n");
	gi.dprintf("-------------------------------------\n");
	...
	/* This is how you set your mod version Ernest */
	gi.cvar("gamever", MODVERSION, CVAR_SERVERINFO | CVAR_LATCH);
	...
	//no_shadows = gi.cvar ("no_shadows", "1", 0);
	
	maxclients = gi.cvar ("maxclients", "4", CVAR_SERVERINFO | CVAR_LATCH);

	// KOOGLEBOT_ADD
	gi.cvar("bots", "", CVAR_SERVERINFO);
	// KOOGLEBOT_END

	// KOOGLEBOT_ADD
	gi.cvar("location", "Florida", CVAR_SERVERINFO);
	// KOOGLEBOT_END

	gi.cvar("antilag", "0", CVAR_SERVERINFO);
	gi.cvar("no_zoom", "0", CVAR_SERVERINFO);
	...
	anti_spawncamp = gi.cvar ("anti_spawncamp","1",CVAR_SERVERINFO);
	...
	// Papa on 10.6.99 used this very basic process of handling files...
	// Message from Alfred Hammer
	// Ghost has chosen to keep this style based on previous releases that
	// Monkey Harris has in part tried to take the responsibility of the initial design!
	// He has added a lot of useful features that have made the game better but all in all
	// he chose to make part of his success a secret, which is frowned upon by other
	// developers looking to learn from the mistakes of others or maybe have the chance to 
	// better something that was written by an inexperienced programmer to start with.
	// Something that is poorly written can always be made better by older folks that
	// have been programming much longer and have a much stronger/honed logic skill set!

	// Xatrix is hated by many as well, all the developers that left a very large 
	// community with their dick in their hands, only one patch after the release v121
	// Secondly many hate gamespy.com more than they hate the selfish shit head developers
	// that once were gray matter studios. Fuck all selfish mod creators and game developers!
	// I'm OUT Alfred Hammer

	// Process/Open the game ini file i.e comp.ini / bloodmoney.ini or hitmen.ini etc
	i = proccess_ini_file();
	...
	if (i != OK)
	{
		...
		{
			gi.dprintf("nbk/config-files/comp.ini file not opened!\n");
		}
		else
		{
			gi.dprintf("Error opening nbk/config-files/comp.ini file!\n");
		}
	}
	else
	{
		gi.dprintf("Processed nbk/config-files/comp.ini\n");
	}
	...
	lockmouse[i]=0;
	for (i=0;i<7;i++) lockrecoil[i]='a'+(rand()%26); // added by FREDZ
	lockrecoil[i]=0;
	for (i=0;i<7;i++) lockangle[i]='a'+(rand()%26);  // added by FREDZ
	lockangle[i]=0;

	for (i = 0; i < 7; i++) ver_check[i] = 'a' + (rand() % 26);  // added by Ghost
	ver_check[i] = 0;
}
