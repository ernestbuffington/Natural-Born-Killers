...
void T_Damage (edict_t *targ, edict_t *inflictor, edict_t *attacker, vec3_t dir, vec3_t point, vec3_t normal, int damage, int knockback, int dflags, int mod)
{
	...
	{
	  //  safe_cprintf (targ, PRINT_HIGH, "1 brace\n");
        if ((OnSameTeam (targ, attacker)) && (!targ->client->pers.friendly_vulnerable))
		{
           // safe_cprintf (targ, PRINT_HIGH, "2 brace\n");
			if ((int)(dmflags->value) & DF_NO_FRIENDLY_FIRE)
			{
             //   safe_cprintf (targ, PRINT_HIGH, "3 brace\n");
				dmg = 0;
			}
			else
            {
              //  safe_cprintf (targ, PRINT_HIGH, "4 brace\n");
        ...
        {
			hit=1;
          //  safe_cprintf (targ, PRINT_HIGH, "5 brace\n");
        }
	} 
    else if (attacker->client && !targ->deadflag && targ!=attacker)
    {
		hit=1;
      //  safe_cprintf (targ, PRINT_HIGH, "6 brace\n");
    }
	meansOfDeath = mod;
	...
	{
       // safe_cprintf (targ, PRINT_HIGH, "7 brace\n");
		if (skill->value == 0)
	...
	{
      //  safe_cprintf (targ, PRINT_HIGH, "8 brace\n");
		dmg *= 4;
	}
...
// figure momentum add
	...
	if (targ->client && dmg && (!(dflags & DAMAGE_NO_ARMOR)))
	{
      ...
      //  safe_cprintf (targ, PRINT_HIGH, "9 brace\n");
        // JOSEPH 21-MAY-99
		if (mod == MOD_DOGBITE)
		{
	    ...
	    if ((item) && (targ->client->pers.inventory[ITEM_INDEX(item)]))
		{
          ...
          //  safe_cprintf (targ, PRINT_HIGH, "10 brace\n");
            ...
            {
              //  safe_cprintf (targ, PRINT_HIGH, "11 brace\n");
			...
			{
              //  safe_cprintf (targ, PRINT_HIGH, "12 brace\n");
			...
			{
			//	safe_cprintf (targ, PRINT_HIGH, "13 brace\n");
	...
	{
       // safe_cprintf (targ, PRINT_HIGH, "14 brace\n");
	...
	{
       // safe_cprintf (targ, PRINT_HIGH, "15 brace\n");
		if (targ->pain_debounce_time < level.time)
		{
         //   safe_cprintf (targ, PRINT_HIGH, "16 brace\n");
			// JOSEPH 29-MAR-99
...
// do the damage
	if (take)
	{
     //  safe_cprintf (targ, PRINT_HIGH, "17 brace\n");

		if (mod == MOD_FLAMETHROWER)
		{
          //  safe_cprintf (targ, PRINT_HIGH, "18 brace\n");
		}
		else
		{
         //   safe_cprintf (targ, PRINT_HIGH, "19 brace\n");
		...
		{
        //    safe_cprintf (targ, PRINT_HIGH, "20 brace\n");
		...
		{
		//	safe_cprintf (targ, PRINT_HIGH, "21 brace\n");
		...
		{
         //   safe_cprintf (targ, PRINT_HIGH, "22 brace\n");
		...
		if (targ->health <= 0)
		{
          ...
          //  safe_cprintf (targ, PRINT_HIGH, "23 brace\n");

            if ((targ->svflags & SVF_MONSTER) || (client))
			{
              //  safe_cprintf (targ, PRINT_HIGH, "24 brace\n");
			...
			if (targ->svflags & SVF_MONSTER)
			{
             //   safe_cprintf (targ, PRINT_HIGH, "25 brace\n");
				// targ->solid = SOLID_NOT;			
				targ->svflags |= SVF_DEADMONSTER;
				
				if (mod == MOD_ROCKET)
				{
				//    safe_cprintf (targ, PRINT_HIGH, "26 brace\n");
...
//			if (!targ->deadflag)
	...
	{
      //  safe_cprintf (targ, PRINT_HIGH, "27 brace\n");

		M_ReactToDamage (targ, attacker, take);
		if (take)
		{
          //  safe_cprintf (targ, PRINT_HIGH, "28 brace\n");
	...
	{
      //  safe_cprintf (targ, PRINT_HIGH, "29 brace\n");
	...
	{
       // safe_cprintf (targ, PRINT_HIGH, "30 brace\n");
	...
	if (client)
	{
      //  safe_cprintf (targ, PRINT_HIGH, "31 brace\n");

		client->damage_armor += asave;
		if (mod == MOD_FLAMETHROWER)
        {
           // safe_cprintf (targ, PRINT_HIGH, "32 brace\n");

			client->damage_flame += take*4;
        }
		else
        {
		//	safe_cprintf (targ, PRINT_HIGH, "33 brace\n");
