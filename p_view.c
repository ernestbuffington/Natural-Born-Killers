...
#define WEAPON_PISTOL	3
	...
	// Ridah, Hovercars
	if (ent->flags & (FL_HOVERCAR | FL_HOVERCAR_GROUND | FL_BIKE))
	{	// always frame 0, use visibility for model_parts to represent damage
		ent->s.frame = 0;
		return;
	}
	// done.
				...
				if (fabs(client->last_climb_anim_z - ent->s.origin[2]) > 1)
				{
					...
					else
					{
						...
						if (client->last_climb_anim_z > ent->s.origin[2])
						{
							...
							{
								int node;
								ent->last_node = KOOGLEND_AddNode(ent, BOTNODE_LADDER);
								node = ent->last_node;
		...
		if (duck)
		{
			if (run < 0)
			{
				...
				case WEAPON_PISTOL:
					...
					{
						int node;
						ent->last_node = KOOGLEND_AddNode(ent, BOTNODE_CROUCH);
						node = ent->last_node;
						gi.dprintf("ent->last_node: %d type: %d x: %f y: %f z %f\n", ent->last_node, nodes[node].type, nodes[node].origin[0], nodes[node].origin[1], nodes[node].origin[2]);
					}
