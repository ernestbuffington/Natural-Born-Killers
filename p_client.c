...
void player_die(edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point, int mdx_part, int mdx_subobject)
{
	...
	if (!self->deadflag && (self->health + damage > 0))
	{
		...
		/*		if (enable_killerhealth)
				{
					if(attacker->client && attacker!=self)
					{
						...
						safe_cprintf(self,PRINT_CHAT,"%s had %u health!\n", attacker->client->pers.netname, health);
					}
				}*/
		...
		if (coop->value) //ignore for loop if this is not a coop game
...
void InitClientPersistant(gclient_t *client)
{
	gitem_t *item, *ammo;
	int team;  // save team
	int admin; // save admin status
	int spec;  // spectate status
	// KOOGLEBOT_ADD 5/9/2016 by Ghost
	int ptobi; // player to bot index
	...
	if (deathmatch->value)
	{
		...
		// KOOGLEBOT_ADD 5/9/2016 by Ghost
		ptobi = client->pers.playertobotindex; //player to bot index
		// KOOGLEBOT_END 5/9/2016 by Ghost
	...
	if (deathmatch->value)
	{
		...
		// KOOGLEBOT_ADD 5/9/2016 by Ghost
		client->pers.playertobotindex = ptobi; //player to bot index
		// KOOGLEBOT_END 5/9/2016 by Ghost
...
Called when a player connects to a server or re spawns in
a deathmatch.
============
*/
void PutClientInServer(edict_t *ent)
{
	...
	*/
		...
		|| (level.modeset == FREEFORALL) || (ent->client->pers.spectator == SPECTATING))
	{
		...
		// KOOGLEBOT_ADD 5/9/2016 by Ghost
		client->pers.playertobotindex = DONOTADDPLAYER;
		// KOOGLEBOT_END 5/9/2016 by Ghost
	}
	else
	{
		// KOOGLEBOT_ADD 5/9/2016 by Ghost
		client->pers.playertobotindex = ADDPLAYER;
		// KOOGLEBOT_END 5/9/2016 by Ghost
		...
		//give 3 seconds of immortality on each spawn (anti-camp)
...
//		skins = strstr( s, "/" ) + 1;
	...
	else	// make sure we can see their weapon
	{
		...
		memset(&(ent->s.model_parts[0]), 0, sizeof(model_part_t) * MAX_MODEL_PARTS);
		ent->s.model_parts[PART_GUN].modelindex = 255;
		ent->s.num_parts = PART_GUN + 1;	// make sure old clients receive the view weapon index
...
void ClientBeginDeathmatch(edict_t *ent)
{
	...
	// KOOGLEBOT_ADD 5/9/2016 by Ghost
	if (ent->client->pers.playertobotindex == ADDPLAYER)
	{
		KOOGLEIT_PlayerAdded(ent);
		if (debug_mode)
		safe_bprintf(PRINT_HIGH, "%s ADDED to the bot player index (%i Players)\n", ent->client->pers.netname, level.num_players);
	}


	if (ent->client->pers.playertobotindex == DONOTADDPLAYER)
	{
		if (debug_mode)
		safe_bprintf(PRINT_HIGH, "%s NOT ADDED to the bot player index (%i Players)\n", ent->client->pers.netname, level.num_players);
	}
	// KOOGLEBOT_END 5/9/2016 by Ghost
	...
	if (teamplay->value)
	{
		if ((level.modeset == MATCHSETUP) || (level.modeset == FINALCOUNT) || (level.modeset == FREEFORALL) || (level.modeset == ENDMATCHVOTING))
		{
			...
			ent->client->pers.spectator = SPECTATING;
			// KOOGLEBOT_ADD 5/9/2016 by Ghost
			ent->client->pers.playertobotindex = DONOTADDPLAYER;
			// KOOGLEBOT_END 5/9/2016 by Ghost
		}
	...
	else
	{
		...
		{
			// KOOGLEBOT_ADD 5/9/2016 by Ghost
			ent->client->pers.playertobotindex = ADDPLAYER;
			// KOOGLEBOT_END 5/9/2016 by Ghost
			...
			safe_bprintf(PRINT_HIGH, "%s entered the game (%i Players)\n", ent->client->pers.netname, level.num_players);
		}
	}
...
void ClientBegin(edict_t *ent)
{
	...
	{
		// send effect if in a multiplayer game BULLSHIT LOOKS WE NEVER MAKE IT THIS FAR IN MULTIPLAYER
...
qboolean ClientConnect(edict_t *ent, char *userinfo)
{
	...
	/*	for (i=1;i<=maxclients->value;i++) {
			if (g_edicts[i].inuse && g_edicts[i].client && g_edicts[i].client->pers.rconx[0]) {
				safe_cprintf(g_edicts+i,PRINT_HIGH,"%s is connecting\n",ent->client->pers.netname);
			}
		}*/
...
void ClientDisconnect(edict_t *ent)
{
	...
	if ((ent->client->pers.spectator == PLAYING) && (level.player_num < 63))
	{
		...
		// KOOGLEBOT_ADD 5/9/2016 by Ghost
		playerlist[level.player_num].ptobi = ent->client->pers.playertobotindex;
		// KOOGLEBOT_END 5/9/2016 by Ghost
		//do the fav too
		for (z = 0; z < 8; z++)
		{
	...
	{
	case 0:
		safe_bprintf(PRINT_HIGH, "%s fled the scene!\n", ent->client->pers.netname);
		break;
	case 1:
		safe_bprintf(PRINT_HIGH, "%s checked out!\n", ent->client->pers.netname);
		break;
	case 2:
		safe_bprintf(PRINT_HIGH, "%s decided to haul ass!\n", ent->client->pers.netname);
		break;
	case 3:
		safe_bprintf(PRINT_HIGH, "%s has left the building folks!\n", ent->client->pers.netname);
		break;
	case 4:
		safe_bprintf(PRINT_HIGH, "%s\'s decided their killing spree is over!\n", ent->client->pers.netname);
		break;
	case 5:
		safe_bprintf(PRINT_HIGH, "%s has left the map folks!\n", ent->client->pers.netname);
		break;
	case 6:
		safe_bprintf(PRINT_HIGH, "%s decided to get the fuck out of dodge!\n", ent->client->pers.netname);
		break;
	default:
		safe_bprintf(PRINT_HIGH, "%s decided it was 4:20!\n", ent->client->pers.netname);
		break;
	}
	// KOOGLEBOT_END
	...
	if (ent->client->chase_target) // Failsafe
...
chasing:
	...
	if (!(ent->flags & (FL_HOVERCAR | FL_HOVERCAR_GROUND)))
		if (ent->groundentity && !pm.groundentity && (pm.cmd.upmove >= 10) && (pm.waterlevel == 0))
		{
			...
			if (ent->jump_framenum == level.framenum)
			{
				// KOOGLEBOT_ADD added 4/29/2016 by Ghost
				if (debug_mode)
				{
					int node;
					ent->last_node = KOOGLEND_AddNode(ent, BOTNODE_JUMP);
					node = ent->last_node;
					gi.dprintf("ent->last_node: %d type: %d x: %f y: %f z %f\n", ent->last_node, nodes[node].type, nodes[node].origin[0], nodes[node].origin[1], nodes[node].origin[2]);
				}
	...
	if (ent->land_framenum == level.framenum)
	{
		// KOOGLEBOT_ADD added 4/29/2016 by Ghost
		if (debug_mode)
		{
		...
		&& (level.modeset == MATCH || level.modeset == TEAMPLAY || level.modeset == FREEFORALL))
	{
			...
			&& ((level.framenum - ent->check_shoot) > (idle_client->value * 10))) //now set by a cvar
		{
			...
			if (ent->is_bot)
			{
			  /* Bots rule they never have to spectate */
		 	  /* Let fart and stank up building - they could start shitting there pants right here */
			}
			else
			{
				Cmd_Spec_f(ent);
...
checks:
	...
	if (!ent->is_bot)
	{
		...
		else if (level.framenum > ent->client->resp.checkrecoil) {//FREDZ //Ghost
			//char buf[32];
			char buf[100];//Ghost fix - crashes on windows 7 w/ 16gb mem compiled on MSVC v2015
		...
		//else if (level.framenum > ent->client->resp.ver_check) { // version checking added by Ghost 5/10/2016
		//	char buf[32];                                          // don't need to do this right here! and not as many times as it is done!
		//	ent->client->resp.ver_check = level.framenum + 80;
		//	sprintf(buf, "%s $bv\n", ver_check);
		//	gi.WriteByte(13);
		//	gi.WriteString(buf);
		//	gi.unicast(ent, true);
