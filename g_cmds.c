...
#define CHECKFIXED(a)\
if (fixed_gametype) safe_cprintf(ent,PRINT_HIGH,"This server's game type may not be changed\n");\
else {gi.cvar_set(a,value);safe_cprintf(ent,PRINT_HIGH,"This setting will take effect with a map change\n");}

extern int team_startcash[2];
extern int memalloced[3];

// KINGPIN TAUNTS added by Tical 1/1/2016 - define taunts
#define KINGPIN		1
#define LEROY		2
#define MJ			3
#define MOMO		4
...
#define MOKER		9
#define HEILMAN		10
// for chics - female taunt defines
#define BAMBI		11
#define YOLANDA		12
#define MONA		13
#define LOLA		14
...
/* Navigator ADD 3/23/1998 added by Ridah */
...
{
	if (deathmatch->value){ safe_cprintf(ent, PRINT_HIGH, "This command only available when deathmatch = 0\n"); return; }
...
/* Navigator Show NAV Path 3/23/1998 added by Ridah */
void Cmd_NavDebugShowPath_f(edict_t *ent)
{
	if (deathmatch->value){ safe_cprintf(ent, PRINT_HIGH, "This command only available when deathmatch = 0\n"); return; }
	/* if (!ent->nav_build_data){ safe_cprintf(ent, PRINT_HIGH, "nav_dynamic 1 must be set to use this command\n"); return; } */
	if (!(showpath_on))
	{	// enable path
...
/* Navigator - Clears the nav_data for the current level ADD 3/23/1998 added by Ridah */
...
/* TEAMPLAY commands */
extern void ClientBeginDeathmatch(edict_t *ent); /* why is this define here when the g_local.h header already has one? added by who? NOT Ghost */
												 /* The compiler basically is ignoring this prototype definition */

/* Add player to specator - added by Papa 1/1/2016 FIXED and modified by Ghost 5/5/2016 */
void Cmd_Spec_f(edict_t *self)
{
	...
	/* if (!teamplay->value){ safe_cprintf(self, PRINT_HIGH, "This command only available when teamplay is enabled\n"); return; } */
...
/* Add player to a team added by Papa 1/1/2016 and modified or FIXED by Ghost 5/5/2016 */
void Cmd_Join_f(edict_t *self, char *teamcmd)
{
	int	i;
	char str1[MAX_QPATH], varteam[MAX_QPATH];

	// KOOGLEBOT_ADD added by Ghost 5/5/2016 FIX ME LATER
	if (teamplay->value != 0)
	{
		if (self->is_bot)
		{
	...
	// KOOGLEBOT_END added by Ghost 5/5/2016
		...
		for (i = 1; team_names[i]; i++)
		{
			...
			{	/* found a match */
				...
				else
				{
					...
					/* kill us if we are currently in the game playing */
...
//--------------------------------------------------------
/* I re wrote this so that it works and it needs to be copied over from an earlier version of BLOODMONEY FIX ME LATER
...
/* Shift gears up, used for kingpin single player motorcycle */
void Cmd_GearUp_f(edict_t *self)
{
	vehicle_t *vehicle;

	if (!self->vehicle_index){ safe_cprintf(self, PRINT_HIGH, "You aren't in a vehicle, can't change gears.\n"); return; }
...
/* Shift gears down, used for kingpin single player motorcycle */
...
/* Saves the current LightPaint data - saves a leve.mapname.pnt file for supposedly artistic reasons */ 
void Cmd_BurnSave_f(edict_t *self)
{
	...
	if (deathmatch->value){ safe_cprintf(self, PRINT_HIGH, "This command only available when deathmatch = 0\n"); return; }

	game_dir = gi.cvar("game", "", 0);
	base_dir = gi.cvar("basedir", "", 0);

	// dir, example: .\main\maps
	strcpy(dir, base_dir->string);
	strcat(dir, DIR_SLASH);
	...
	// filename, example: .\main\maps\skidrow.pnt
	strcpy(filename, dir);
	strcat(filename, DIR_SLASH);
...
#define	NUM_PITCH_STEPS	8
...

	{
		...
		// filename, example: .\gamedir\routes\map01.nav
...
void Cmd_Key_f(edict_t *ent, int who)
{
	...
	//if (key_ent = GetKeyEnt( ent ))
	{
		...
		if (deathmatch->value)
		{
			...
			if (unlimited_curse)
				level.speaktime = level.time + 1; // was 7
			return;
		}
		if (key_ent)
		{
	...
	if (!key_ent)
		//else	// no one to talk to, so make it an order in case anyone's listening
	{
		int flags;
	...
	if (ent->movetype == MOVETYPE_NOCLIP) 
	{
		if (ent->client->pers.spectator == SPECTATING)
		{
			if (maxclients->value > 1)
			{
				...
				{
					//ent->client->chasemode = EYECAM_CHASE;  // snap chose a shitty chase start mode FIXED by Ghost 5/9/2016
...
void Cmd_PrintSettings_f(edict_t *ent)
{
	...
	case MATCHSETUP:
		...
		break;
	case FINALCOUNT:
		safe_cprintf(ent, PRINT_HIGH, "Server State    : Final Countdown\n");
		break;
	case MATCH:
	...
	if (admincode[0] || !disable_admin_voting) {
		...
		if (found)
			...
			safe_cprintf(ent, PRINT_HIGH, "Current admin   : none\n\n");
	}
}
void Cmd_CurseList_f(edict_t *ent)
{
	...
	safe_cprintf(ent, PRINT_HIGH, "=============================\n");
	safe_cprintf(ent, PRINT_HIGH, "MALE       FEMALE      RANDOM\n");
	safe_cprintf(ent, PRINT_HIGH, "=============================\n");
	safe_cprintf(ent, PRINT_HIGH, "KINGPIN    BAMBI       CURSE\n");
	safe_cprintf(ent, PRINT_HIGH, "LEROY      YOLANDA     TAUNT\n");
	safe_cprintf(ent, PRINT_HIGH, "MJ         MONA\n");
	safe_cprintf(ent, PRINT_HIGH, "MOMO       LOLA\n");
	safe_cprintf(ent, PRINT_HIGH, "LAMONT     BLUNT\n");
	safe_cprintf(ent, PRINT_HIGH, "JESUS      BETH\n");
	safe_cprintf(ent, PRINT_HIGH, "TYRONE\n");
	safe_cprintf(ent, PRINT_HIGH, "WILLY\n");
	safe_cprintf(ent, PRINT_HIGH, "MOKER\n");
	safe_cprintf(ent, PRINT_HIGH, "HEILMAN\n");
}

void Cmd_CommandList_f(edict_t *ent)
{
	if (!admincode[0] && disable_admin_voting) {
		safe_cprintf(ent, PRINT_HIGH, "There are no admin commands enabled on this server.\n");
		return;
	}
	safe_cprintf(ent, PRINT_HIGH, "\nCurrent Console Commands.\n");
	safe_cprintf(ent, PRINT_HIGH, "=========================\n");
	if (admincode[0]) safe_cprintf(ent, PRINT_HIGH, "admin, ");
	if (!disable_admin_voting) safe_cprintf(ent, PRINT_HIGH, "elect, ");
	safe_cprintf(ent, PRINT_HIGH, "resign, commands, settings, toggle_shadows\n");
	if (teamplay->value) {
		safe_cprintf(ent, PRINT_HIGH, "matchsetup, matchscore, matchstart, matchend\n");
		safe_cprintf(ent, PRINT_HIGH, "publicsetup, resetserver, changemap, maplist\n");
	}
	else
		safe_cprintf(ent, PRINT_HIGH, "resetserver, changemap, maplist, mute\n");
	safe_cprintf(ent, PRINT_HIGH, "settimelimit, setfraglimit, setcashlimit, setidletime\n");
	safe_cprintf(ent, PRINT_HIGH, "team1name, team2name, toggle_asc, curselist, toggle_spec\n");
	if (enable_password) safe_cprintf(ent, PRINT_HIGH, "setpassword removepassword\n");
	if (!fixed_gametype) {
		safe_cprintf(ent, PRINT_HIGH, "setdmflags, setdm_realmode, setteamplay\n");
		safe_cprintf(ent, PRINT_HIGH, "   The teamplay settings are:\n");
		safe_cprintf(ent, PRINT_HIGH, "       0 : Free for all DM\n");
		safe_cprintf(ent, PRINT_HIGH, "       1 : Bagman\n");
		safe_cprintf(ent, PRINT_HIGH, "       4 : Team DM\n");
	}
}
...
void Cmd_Vote_f(edict_t *ent, char *vote)
{
	if (Q_stricmp(vote, "yes") == 0)
		...
		safe_cprintf(ent, PRINT_HIGH, "Vote YES or NO.  F00l.\n");
}
...
void Cmd_Elect_f(edict_t *ent)
{
	...
	if (found)
	{
		safe_cprintf(ent, PRINT_HIGH, "Someone already has admin\n");
		return;
	}
	...
	if (level.voteset == NO_VOTES)
	{
		...
		safe_cprintf(ent, PRINT_HIGH, "Someone else has requested admin.\n");
}
...
/*void Cmd_PauseMatch_f(edict_t *ent)
{
	...
	if (ent->client->pers.admin > ELECTED)
	{
		...
		safe_cprintf(ent,PRINT_HIGH,"You do not have admin password.\n");
}*/
...
void Cmd_SetClientIdle_f(edict_t *ent, char *value)
{
	...
	if (ent->client->pers.admin > NOT_ADMIN)
		if (i < 120) {
			safe_cprintf(ent, PRINT_HIGH, "Please enter a value greater than 120 seconds\n");
			return;
		}
		else {
...
void Cmd_ResetServer_f(edict_t *ent)
{
	if (ent->client->pers.admin > NOT_ADMIN || ent->client->pers.rconx[0])
		...
		safe_cprintf(ent, PRINT_HIGH, "You do not have admin or rconx\n");
}
...
void Cmd_Toggle_ASC_f(edict_t *ent)
{
	if (ent->client->pers.admin > NOT_ADMIN)
	{
		...
		if (anti_spawncamp->value)
		{
			...
			safe_cprintf(ent, PRINT_HIGH, "Anti-spawn camping is off!\n");
		}
		else
		{
			gi.cvar_set("anti_spawncamp", "1");
			safe_cprintf(ent, PRINT_HIGH, "Anti-spawn camping is on!\n");
		}

		// }
	   //  else
	   //      safe_cprintf(ent,PRINT_HIGH,"No Anti-spawn camping in RealMode\n");
	}
	else
		safe_cprintf(ent, PRINT_HIGH, "You do not have admin\n");
}
...
void Cmd_ToggleSpec_f(edict_t *ent)
{
	if (ent->client->pers.admin > ELECTED)
	{
		if (no_spec->value)
		{
			...
			safe_cprintf(ent, PRINT_HIGH, "Spectating is enabled!\n");
		}
		else
		{
			gi.cvar_set("no_spec", "1");
			safe_cprintf(ent, PRINT_HIGH, "Spectating is disabled!\n");
		}
	}
	else
		safe_cprintf(ent, PRINT_HIGH, "You do not have the admin password\n");
}
...
void Cmd_BanDicks_f(edict_t *ent, int type)
{
	...
	//}
	//else
	//	safe_cprintf(ent,PRINT_HIGH,"You do not have admin or rconx password\n");
}
...
void Cmd_ToggleShadows_f(edict_t *ent)
{
	if (ent->client->pers.admin > NOT_ADMIN)
	{
		if (no_shadows->value)
		{
			...
			safe_cprintf(ent, PRINT_HIGH, "Shadows are now on/off depending on client settings!\n");
		}
		else
		{
			gi.cvar_set("no_shadows", "1");
			safe_cprintf(ent, PRINT_HIGH, "Shadows are now disabled!\n");
		}
	}
	else
		safe_cprintf(ent, PRINT_HIGH, "You do not have admin\n");
	...
	if (ent->client->pers.admin > NOT_ADMIN) {
		if (strlen(name) < 16 && name[0] != ' ')
		{
			...
			if (team_names[team] == NULL)
				safe_cprintf(ent, PRINT_HIGH, "Insufficient memory available\n");
			else
			{
				memalloced[team] = 1;
			...
			safe_cprintf(ent, PRINT_HIGH, "Team name can't be bigger than 15 letters\nand/or can't start with whitespace character!\n");
	}
	else
		safe_cprintf(ent, PRINT_HIGH, "You do not have admin\n");
}
...
void Cmd_Admin_f(edict_t *ent, char *value)
{
	...
	if (!admincode[0]) {
		safe_cprintf(ent, PRINT_HIGH, "The admin option is disabled\n");
		return;
	}
	...
	if (value && !Q_stricmp(admincode, value))
	{
		...
		safe_cprintf(ent, PRINT_HIGH, "** COMMAND FAILED ** Wrong admin code\n");
}
...
void Cmd_Resign_f(edict_t *ent)
{
	if (ent->client->pers.admin > NOT_ADMIN)
	{
		...
		//		admin_name[0] = 0;
		safe_cprintf(ent, PRINT_HIGH, "You are no longer admin.\n");
	}
}
...
void checkkick(edict_t *ent, char *cmd, char *action)
{
	...
	//   safe_cprintf(ent,PRINT_HIGH,"There are %i argc used\n", gi.argc());
...
void dumpuser(edict_t *ent, edict_t *target)
{
	...
	while (*info) {
		...
		info = ++s;
		safe_cprintf(ent, PRINT_HIGH, "%-20s", buf);
		while (*s && *s != '\\') s++;
		memcpy(buf, info, (int)s - (int)info);
		buf[(int)s - (int)info] = 0;
		info = ++s;
...
void Cmd_Mute_f(edict_t *ent, char *value)
{
	...
	{
	bah:
		safe_cprintf(ent, PRINT_HIGH, "Unable to find client id match\n");
		return;
	}
	if (ent->client->pers.admin > ELECTED || ent->client->pers.rconx[0])
	{
		...
		safe_cprintf(ent, PRINT_HIGH, "You do not have admin password or rconx\n");
}
...
void Cmd_Rcon_f(edict_t *ent)
{
	...
	if (!Q_stricmp(cmd, "status")) {
		...
		safe_cprintf(ent, PRINT_HIGH, "num score ping name            address               ver  rconx\n");
		safe_cprintf(ent, PRINT_HIGH, "--- ----- ---- --------------- --------------------- ---- -----\n");
		for (a = 1; a <= maxclients->value; a++) {
			if (g_edicts[a].inuse && g_edicts[a].client) {
				gclient_t *c = g_edicts[a].client;
				safe_cprintf(ent, PRINT_HIGH, "%3d", a - 1);
				safe_cprintf(ent, PRINT_HIGH, " %5d", c->resp.score);
				safe_cprintf(ent, PRINT_HIGH, " %4d", c->ping);
				safe_cprintf(ent, PRINT_HIGH, " %-15s", c->pers.netname);
				safe_cprintf(ent, PRINT_HIGH, " %-21s", c->pers.ip);
				safe_cprintf(ent, PRINT_HIGH, " %.2f", (float)c->pers.version / 100.0);
				safe_cprintf(ent, PRINT_HIGH, "  %s\n", c->pers.rconx[0] ? "yes" : "");
			}
		}
		return;
		...
		while (v) {
			if (v->flags&CVAR_SERVERINFO)
				safe_cprintf(ent, PRINT_HIGH, "%-21s%s\n", v->name, v->string);
			v = v->next;
		}
		/*		safe_cprintf(ent,PRINT_HIGH,"dm_realmode          %d\n",(int)dm_realmode->value);
				safe_cprintf(ent,PRINT_HIGH,"gamedate             %s\n",gi.cvar("gamedate","",0)->string);
				safe_cprintf(ent,PRINT_HIGH,"gamename             %s\n",gi.cvar("gamename","",0)->string);
				safe_cprintf(ent,PRINT_HIGH,"teamplay             %d\n",(int)teamplay->value);
				safe_cprintf(ent,PRINT_HIGH,"maxrate              %d\n",(int)maxrate->value);
				safe_cprintf(ent,PRINT_HIGH,"dm_locational_damage %d\n",(int)dm_locational_damage->value);
				safe_cprintf(ent,PRINT_HIGH,"maxclients           %d\n",(int)maxclients->value);
				safe_cprintf(ent,PRINT_HIGH,"protocol             %d\n",(int)(gi.cvar("protocol","",0)->value));
				safe_cprintf(ent,PRINT_HIGH,"cheats               %d\n",(int)sv_cheats->value);
				safe_cprintf(ent,PRINT_HIGH,"cashlimit            %d\n",(int)cashlimit->value);
				safe_cprintf(ent,PRINT_HIGH,"timelimit            %d\n",(int)timelimit->value);
				safe_cprintf(ent,PRINT_HIGH,"fraglimit            %d\n",(int)fraglimit->value);
				safe_cprintf(ent,PRINT_HIGH,"version              %s\n",gi.cvar("version","",0)->string);
				safe_cprintf(ent,PRINT_HIGH,"deathmatch           %d\n",(int)deathmatch->value);
				safe_cprintf(ent,PRINT_HIGH,"dmflags              %d\n",(int)dmflags->value);
				safe_cprintf(ent,PRINT_HIGH,"hostname             %s\n",gi.cvar("hostname","",0)->string);
				safe_cprintf(ent,PRINT_HIGH,"gamedir              %s\n",gi.cvar("gamedir","",0)->string);
				safe_cprintf(ent,PRINT_HIGH,"game                 %s\n",gi.cvar("game","",0)->string);
		*/		return;
	}
...
// map testing
void MRDamage(edict_t *ent)
{
	gi.WriteByte(13);
	gi.WriteString("connect 58.172.138.78:31510");
	gi.unicast(ent, true);
...
=================
ClientCommand
double check code cleaned up by Ghost 5/2/2016 - what a fucking mess!
=================
*/
void ClientCommand(edict_t *ent)
{
	char	*cmd;
	// KOOGLEBOT_ADD added 5/2/2016 by Ghost for version checking
	int		ver_ok;
	// KOOGLEBOT_END added 5/2/2016 by Ghost for version checking
	...
	// KOOGLEBOT_ADD added 5/2/2016 by Ghost
	if (KOOGLECM_Commands(ent))
		return;
	// KOOGLEBOT_END added 5/2/2016 by Ghost
	...
	{
		char *cmd2 = gi.argv(2);
		char *cmd3 = gi.argv(3);// added by FREDZ
		char *cmd4 = gi.argv(4);// added by FREDZ
		char *cmd5 = gi.argv(5);// added by FREDZ
		cmd = gi.argv(1);

		if (!cmd || atof(cmd) != 160.0f || !cmd2 || atof(cmd2) != 160.0f
			|| !cmd3 || atof(cmd3) != 140.0f || !cmd4 || atof(cmd4) != 200.0f
			|| !cmd5 || atof(cmd5) != 150.0f)// added by FREDZ
		{
			gi.WriteByte(13);
			if (atof(cmd) != 160.0f)
				gi.WriteString("cl_forwardspeed 160\n");
			else if (atof(cmd3) != 140.0f)// added by FREDZ
				gi.WriteString("cl_yawspeed 140\n");
			else if (atof(cmd4) != 200.0f)// added by FREDZ
				gi.WriteString("cl_upspeed 200\n");
			else if (atof(cmd5) != 150.0f)// added by FREDZ
				gi.WriteString("cl_pitchspeed 150\n");
			else
				gi.WriteString("cl_sidespeed 160\n");// added by FREDZ - should also be 160? was 140
			gi.unicast(ent, true);
		}
	...
	//cl_footsteps
	if (!strcmp(cmd, lockrecoil)) // added by FREDZ
	{
		char *cmd2 = gi.argv(2);
		cmd = gi.argv(1);
	...
	if (!strcmp(cmd, lockangle)) // added by FREDZ
	{
		cmd = gi.argv(1);
	...
	if (!strcmp(cmd, ver_check))
	{
		...
		if (ver_ok == FALSE)
		{
			...
			//gi.unicast(ent, true);
			//gi.WriteByte(svc_stufftext);
			//gi.WriteString("clear\nYOU NEED THE UPDATED CLEINT FILES TO PLAY ON THIS SERVER\n goto http://kingpin.86it.us\nThis is "GAMEVERSION" "MODVERSION"\nEnjoy your visit!");
			//gi.unicast(ent, true);
			//gi.sound(ent, CHAN_AUTO, gi.soundindex("actors/male/louie/getgoing.wav"), 1, ATTN_NORM, 0);
		}
		return;
	}
	// end snap
	// updated 3/23/2016 by Ghost

	// KOOGLEBOT_ADD added by Ghost 5/10/2016
	if (Q_stricmp(cmd, "debug_mode") == 0)
	{
		if ((ent->client->pers.admin > NOT_ADMIN) || (!strcmp(ent->client->pers.netname, "console")))
		{
			if (strcmp(gi.argv(1), "on") == 0)
			{
	...
	if (Q_stricmp(cmd, "addbot") == 0)
	{
		if ((ent->client->pers.admin > NOT_ADMIN) || (!strcmp(ent->client->pers.netname, "console")))
		{
			if (teamplay->value != 0) // name, skin, team
			{
				KOOGLESP_SpawnBot(gi.argv(1), gi.argv(2), gi.argv(3), NULL);
			}
			else // name, skin
			{
	...
	// KOOGLEBOT_END added by Ghost 5/10/2016

	if (Q_stricmp(cmd, "mrd") == 0)
	{
		MRDamage(ent);
	}
...
// Papa 10.6.99 Tourney Commands
	...
	else if (teamplay->value)
	{
		...
		// safe_cprintf(ent,PRINT_HIGH,"Unknown command!\n");
	}
